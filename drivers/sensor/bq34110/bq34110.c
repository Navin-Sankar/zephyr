				...
				uint8_t reg_addr, int16_t *val)
{
	...
	ret = i2c_write_read(bq34110->i2c, DT_INST_REG_ADDR(0), &reg_addr,
			     sizeof(reg_addr), &i2c_data, sizeof(i2c_data));
	if (ret < 0) {
		return ret;
	}
				  ...
				  uint16_t subcommand)
{
	...
	write_data[0] = MANUFACTURER_ACCESS_CONTROL;
	write_data[1] = (uint8_t)(subcommand & 0x00FF);
	write_data[2] = (uint8_t)((subcommand >> 8) & 0x00FF);
	ret = i2c_write(bq34110->i2c, write_data, sizeof(write_data),
			DT_INST_REG_ADDR(0));
	if (ret < 0) {
		return ret;	
	}
...
{
	int ret;
	uint8_t read_reg, write_data[3], read_data[2];
	...
	ret = i2c_write_read(bq34110->i2c, DT_INST_REG_ADDR(0), &read_reg,
			     sizeof(read_reg), &read_data, sizeof(read_data));
	if (ret < 0) {
		return ret;
	}
				...
				enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_GAUGE_REMAINING_CHARGE_CAPACITY:
		ret = bq34110_cmd_reg_read(bq34110, REMAINING_CHARGE_CAPACITY,
			     ...
			     uint8_t data[], uint8_t datalength)
{
	int i, ret;
	uint16_t datasum;
	uint8_t i2c_datasum_datalen[3], i2c_df_addr_data[3],
	...
	ret = i2c_write(bq34110->i2c, i2c_df_addr_data, 
			sizeof(i2c_df_addr_data), DT_INST_REG_ADDR(0));
	if (ret < 0) {
		return ret;
	}
	...
	ret = i2c_write(bq34110->i2c, i2c_datasum_datalen,
			sizeof(i2c_datasum_datalen), DT_INST_REG_ADDR(0));
	if (ret < 0) {
		return ret;
	}
...
static int bq34110_gauge_init(const struct device *dev)
{
	...
	uint16_t design_voltage, taper_current, taper_voltage, design_capacity;

	printk("Feul Gauge sensor Init\n");
	...
	ret = updateDFparameter(bq34110, DESIGN_VOLTAGE, writedata,
				sizeof(writedata));
	if (ret < 0) {
	...
	ret = updateDFparameter(bq34110, DESIGN_CAPACITY_MAH, writedata,
				sizeof(writedata));
	if (ret < 0) {
	...
	ret = updateDFparameter(bq34110, OPERATION_CONFIG_A, writedata,
				sizeof(writedata));
	if (ret < 0) {
	...
	ret = updateDFparameter(bq34110, TAPER_CURRENT, writedata,
				sizeof(writedata));
	if (ret < 0) {
	...
	ret = updateDFparameter(bq34110, TAPER_VOLTAGE, writedata,
				sizeof(writedata));
	if (ret < 0) {
	...
	printk("Columb Calibration && Board Offset done\n");
	...
	printk("Obtained Raw Calibration data done\n");
	...
	printk("Voltage Calibration done\n");
	...
	printk("Current Calibration done\n");
	...
	printk("Temperature Calibration done\n");
	*/
/*
	ret = bq34110_ctrl_reg_write(bq34110, PIN_VEN_RESET);
	if (ret < 0) {
		LOG_ERR("Failed to select internal volatge divider");
		return ret;
	}

	ret = bq34110_ctrl_reg_write(bq34110, PIN_LEN_SET);
	if (ret < 0) {
		LOG_ERR("Failed to enable len gpio");
		return ret;
	}
*/
#if BQ34110_TRIGGER_ENABLED
	if (bq34110_init_interrupt(bq34110) < 0) {
...
#ifdef CONFIG_BQ34110_TRIGGER
#define BQ34110_INTERRUPT_INIT(index)					      \
	static const struct bq34110_config bq34110_config_##index = {	      \
		.alert_pin = DT_INST_GPIO_PIN(0, alert_gpio),		      \
		.alert_flag = DT_INST_GPIO_FLAGS(0, alert_gpio),	      \
		.alert_controller = DT_INST_GPIO_LABEL(0, alert_gpio),	      \
	};
